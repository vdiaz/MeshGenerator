...
{  GUIDialogRefinar = this;
   ...
   Setting & algoritmosRefinar = cfg_reader->getRefinarMalla()["Algoritmos"];
   bool leppDelaunay, longestEdge, setActive = true;
   algoritmosRefinar.lookupValue("LeppDelaunay", leppDelaunay);
   algoritmosRefinar.lookupValue("LongestEdge", longestEdge);
   ...
   if (longestEdge) {
      ...
      radiobuttonDelaunayLongestEdgeBisection->set_active(setActive);
   }
   ...
   Setting & criteriosRefinar = cfg_reader->getRefinarMalla()["Criterios"];
   bool anguloMin, areaMax, largoMax;
   criteriosRefinar.lookupValue("AnguloMin", anguloMin);
   criteriosRefinar.lookupValue("AreaMax", areaMax);
   criteriosRefinar.lookupValue("LargoMax", largoMax);
   ...
   if (areaMax) {
      radiobuttonAreaCara->set_active(setActive);
      radiobuttonAreaCara->show();
      spinbuttonAreaCara->set_sensitive(setActive);
   ...
   if (largoMax) {
      radiobuttonLargoArcos->set_active(setActive);
      radiobuttonLargoArcos->show();
      spinbuttonLargoArcos->set_sensitive(setActive);
