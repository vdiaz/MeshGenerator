	...
	while(seEncontroInterseccion==true){
    	...
    	for(int i=0; i<caras->getNumCaras(); i++) {
    	    if(malla->getCara(i)!=NULL && !criterio->seCumple(malla,i)) {
    	        cout << "Aplicando Refinamiento a cara " << i << " de " << caras->getNumCaras() << " (" << (float)i/caras->getNumCaras()*100 << "%)" <<endl;
    	        Improve(malla,i);
...
void LeppDelaunay::BoundaryEdgePointInsertion(Malla *malla, int ind_arco){
    ...
    if(ind_c1 != -1) {
        ind_a1 = malla->getArcoOpuestoNodo(ind_c1,ind_p1);
        ind_a2 = malla->getArcoOpuestoNodo(ind_c1,ind_p2);
    	}
    if(ind_c2 != -1) {
        ind_a1 = malla->getArcoOpuestoNodo(ind_c2,ind_p1);
        ind_a2 = malla->getArcoOpuestoNodo(ind_c2,ind_p2);
    	}
    ...
    malla->makeDelaunay(ind_a1);
    malla->makeDelaunay(ind_a2);
	}
...
void LeppDelaunay::TerminalEdgePointInsertion(Malla *malla, int ind_arco){
    ...
    ind_a1_c1 = malla->getArcoOpuestoNodo(ind_c1,ind_p1);
    ind_a2_c1 = malla->getArcoOpuestoNodo(ind_c1,ind_p2);
    ind_a1_c2 = malla->getArcoOpuestoNodo(ind_c2,ind_p1);
    ind_a2_c2 = malla->getArcoOpuestoNodo(ind_c2,ind_p2);
    ...
    malla->makeDelaunay(ind_a1_c1);
    malla->makeDelaunay(ind_a2_c1);
    malla->makeDelaunay(ind_a1_c2);
    malla->makeDelaunay(ind_a2_c2);
	}

void LeppDelaunay::PointInsertion(Malla *malla, int ind_cara){
	Lepp objetoLepp;
	vector<int> leep = objetoLepp.findLepp(malla, ind_cara);
    int arco_terminal = malla->getArcoMasLargo(leep[leep.size()-1]);
    ...
    for(int i=1; i<(int)leep.size(); i++) {
        if(malla->esCaraBorde(leep[i])) {
            ...
            for(int j=0; j<(int)arcos_borde.size(); j++) {
                if(arcos_borde[j] != malla->getArcoMasCorto(leep[i])) {
                    boundary_t = leep[i];
...
void LeppDelaunay::Improve(Malla *malla, int ind_cara){
	Cara *bad_cara = malla->getCaras()->getCara(ind_cara);
    Cara *new_cara = malla->getCaras()->getCara(ind_cara);
    while(bad_cara == new_cara) { //while bad_cara remains without being modified DO
        ...
        if(malla->esCaraBorde(ind_cara)) {
            ...
            for(int i=0; i<(int)arcos_borde.size(); i++) {
                if(arcos_borde[i] != malla->getArcoMasCorto(ind_cara)) {
                    condicion = true;
        ...
        new_cara = malla->getCaras()->getCara(ind_cara);
    	}
	}
