...
Malla* GeneraFromMatLab::generaMallaTriangulos() {
    ...
    //int num_caras = 2*(num_anillos-1)*num_puntos; // número de caras de la malla
    //int num_arcos = 3*(num_anillos-1)*num_puntos + num_puntos; // número de arcos de la malla
    ...
    for(int j=0; j<4; j++) {
        for(int i=0; i<num_nodos; i++) {
            ...
            if(j == 0) {
                ...
                nodos->getNodo(i)->getPunto()->setY(flt);
            }
            else if(j == 2) {
                nodos->getNodo(i)->getPunto()->setZ(flt);
            }
            else {
    ...
    for(int i=0; i<(num_anillos-1); i++) {
        for(int j=0; j<num_puntos; j++) {
            ...
            c = new Cara(ind_nodos,ind_arcos);
